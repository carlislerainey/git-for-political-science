...
* **Reproducibility**. The ability of others to repropduce your results is crucial to scientific credibility. By "reproducible," I simply mean that other researchers can take the original (raw) data and use the same techniques to generate your results. If this minimal standard is not met, then your research is not credible. Too often, "replication files" contain the final cleaned data set and the script to run the final model. In my opinion, this does not meet the minimal standard and is an inefficient way for science to proceed. Publicly version-controlled research makes the *entire* project available to other researchers. That includes all the data (original, raw, messy data and cleaned versions), all the code (including code to do the data cleaning), and all the text produced as part of writing the manuscript. 
